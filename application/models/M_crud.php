...
class M_crud extends CI_Model{
	...
	function detail_syarat($id_bansos)
{
	$result = $this->db->where('id_bansos', $id_bansos)->get('tbl_jenisbansos');
...
function detail_bansos($id_data)
{
	$result = $this->db->where('id_data', $id_data)->get('tbl_databansos');
	if($result->num_rows() > 0){
		return $result->result();
	}else{
		return false;
...
function detail_biodata($id_pegawai)
{
	$result = $this->db->where('id_pegawai', $id_pegawai)->get('tbl_pegawai');
	if($result->num_rows() > 0){
		return $result->result();
	}else{
		return false;
...
{
	$result = $this->db->get('tbl_databansos');
	 	if($result->num_rows()> 0){
	 		return $result->result();
	 	}else{
	 		return FALSE;
...
public function data_tidakmampu()
{
	return $this->db->get_where('tbl_databansos', array('jenis' => 'Tidak Mampu'));
}

public function data_blt()
{
	return $this->db->get_where('tbl_databansos', array('jenis' => 'BLT'));
}

public function data_umkm()
{
	return $this->db->get_where('tbl_databansos', array('jenis' => 'UMKM'));
}

public function data_bnpt()
{
	return $this->db->get_where('tbl_databansos', array('jenis' => 'BNPT'));
}

public function data_pengumuman()
{
	$result = $this->db->get('tbl_pengumuman');
	 	if($result->num_rows()> 0){
	 		return $result->result();
	 	}else{
	 		return FALSE;
...
{
	$result = $this->db->where('id_pengumuman', $id_pengumuman)->get('detail_pengumuman');
	if($result->num_rows() > 0){
		return $result->result();
	}else{
		return false;
